...
# TalentOz HCM - Free Tier Deployment Script (FIXED)
...
deploy_infrastructure() {
    ...
    # FIXED: Remove existing main.tf and use only free-tier configuration
    if [ -f "main.tf" ]; then
        log_info "Backing up existing main.tf..."
        mv main.tf main-production.tf.backup
    fi
    
    # Use free-tier configuration as main.tf
...
deploy_applications() {
    ...
    log_info "Waiting for applications to be ready (this may take 5-10 minutes)..."
...
create_free_tier_manifests() {
    ...
    ERPNEXT_DB_PASSWORD=$(cd terraform && terraform output -raw erpnext_db_password)
